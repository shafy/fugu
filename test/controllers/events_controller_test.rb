...
class EventsControllerTest < ActionDispatch::IntegrationTest
  class GetIndex < EventsControllerTest
    ...
    test "to show index if no live events" do
      ...
      sign_in @user
      get user_project_events_path(@user.hash_id, @project.name)
      assert :success
    end
    ...
    test "to show index if no test events" do
      ...
      sign_in @user
      get user_project_events_path(@user.hash_id, @project.name, params: { test: true })
      assert :success
    end
    ...
    test "to redirect to show if events in live mode" do
      ...
      sign_in @user
      get user_project_events_path(@user.hash_id, @project.name, params: { test: false })
      assert_redirected_to user_project_event_path(@user.hash_id,
                                                   @project.name,
                                                   live_event.name.parameterize,
                                                   params: { test: false })
    end
  ...
  class GetShow < EventsControllerTest
    ...
    test "be successful" do
      setup_live_events
      sign_in @user
      get user_project_event_path(@user.hash_id, @project.name, @event.name.parameterize)
      assert_response :success
    end

    test "be successful for public project" do
      setup_live_events
    ...
    test "don't contain footer for public project" do
      setup_live_events
      @project.update(public: true)
      get user_project_event_path(@user.hash_id, @project.name, @event.name.parameterize)
    ...
    test "to redirect to index if no event with this name (test mode)" do
      live_event = FactoryBot.create(:event, api_key: @api_key_live)
      sign_in @user
      get user_project_event_path(@user.hash_id, @project.name, live_event.name.parameterize,
                                  params: { test: true })
      assert_redirected_to user_project_events_path(@user.hash_id, @project.name,
                                                    params: { test: true })
    end
    ...
    test "to redirect to index if no event with this name (live mode)" do
      test_event = FactoryBot.create(:event, api_key: @api_key_test)
      sign_in @user
      get user_project_event_path(@user.hash_id, @project.name, test_event.name.parameterize,
                                  params: { test: false })
      assert_redirected_to user_project_events_path(@user.hash_id, @project.name,
                                                    params: { test: false })
    end
